...
for file in args.test_csv:
    ...
    for filename in os.listdir(folder):
        ...
        if filepath.endswith('.tor'):
            model = ProteinNet(fea_dim=2048, hidden_dim1=128, hidden_dim2=64, num_layers=3)
            try:
            ...
            for i in range(data.shape[0]):
               wt_features = data.iloc[i, :2048]
               mt_features = data.iloc[i, 2051:]
               true_ddg = data.iloc[i, 2048]
               pdbname = data.iloc[i, 2049]
               mutation = data.iloc[i, 2050]
