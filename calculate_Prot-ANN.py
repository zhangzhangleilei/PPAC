...
for file in args.test_csv:
    ...
    for filename in os.listdir(folder):
        ...
        if filepath.endswith('.tor'):
            ...
            model = ProteinNet(fea_dim=1024, hidden_dim1=128, hidden_dim2=256, num_layers=3)
            try:
            ...
            for i in range(data.shape[0]):
                wt_features = data.iloc[i, :1024]
                mt_features = data.iloc[i, 1027:]
                ...
                true_ddg = data.iloc[i, 1025]
                pdbname = data.iloc[i, 1024]
                mutation = data.iloc[i, 1026]
                pdb.append(pdbname)
                mut.append(mutation)
