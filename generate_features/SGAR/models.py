...
class GeometricEncoder(torch.nn.Module):
    ...
    def gen_features(self, X, E, Ea, X_m, E_m, Ea_m):
        ...
        wt_rep1 = [maxx4, meanx4, maxx4_contact, meanx4_contact,
                maxx3, meanx3,  maxx3_contact, meanx3_contact]

        wt_rep=torch.cat(wt_rep1,0) # (2d

        mt_rep1 = [maxx4m, meanx4m, maxx4m_contact, meanx4m_contact,
                   maxx3m, meanx3m, maxx3m_contact, meanx3m_contact]

        mt_rep = torch.cat(mt_rep1, 0)  # (2d
        return wt_rep,mt_rep
        # rep1 = [maxx4, maxx4m, meanx4, meanx4m, maxx4_contact, maxx4m_contact, meanx4_contact, meanx4m_contact, \
        #         maxx3, maxx3m, meanx3, meanx3m, maxx3_contact, maxx3m_contact, meanx3_contact, meanx3m_contact]
        # #
...
# def GeoPPIfeature(A, E, A_m, E_m, model, sorted_idx, flag):
#     with torch.no_grad():
#         wt_fea,mt_fea = model.gen_features(A, E, E, A_m, E_m, E_m)
#
#     wt_features = np.round(wt_fea.cpu().view(1, -1).numpy(), 3)
#     mt_features = np.round(mt_fea.cpu().view(1, -1).numpy(), 3)

    # x=features[:, sorted_idx[:240]]


    # return wt_features,mt_features
